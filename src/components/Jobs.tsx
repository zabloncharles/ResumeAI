...
const HERO_TAGS = ['Developer', 'Designer', 'Marketing', 'Product', 'QA', 'Data'];
...
const Jobs = () => {
  ...
  const jobTypeLabel = (type?: string) => {
    switch (type) {
      ...
      case 'contract': return 'Project Work';
  ...
  const filteredJobs = useMemo(() => {
    let filtered = jobs.filter(job => {
      ...
      if (hideNoSalary && (!job.salary || job.salary.trim() === '')) return false;
      return true;
    });
    // Sort
    if (sort === 'recent') {
      filtered = filtered.sort((a, b) => new Date(b.publication_date).getTime() - new Date(a.publication_date).getTime());
    } else if (sort === 'high') {
      filtered = filtered.sort((a, b) => (parseSalary(b.salary || '') || 0) - (parseSalary(a.salary || '') || 0));
    } else if (sort === 'low') {
      filtered = filtered.sort((a, b) => (parseSalary(a.salary || '') || 0) - (parseSalary(b.salary || '') || 0));
    }
    ...
    return filtered;
  }, [jobs, selectedTypes, selectedCategories, salaryRange, sort, hideNoSalary, entryLevelOnly, userCountry]);
    ...
    <>
      ...
      <div className="min-h-screen bg-[#fafbfc] flex flex-col items-center justify-start py-8 px-2">
        ...
        <section className="w-full max-w-3xl mx-auto text-center py-16">
          <div className="mb-2 text-xs tracking-widest text-gray-400 font-semibold">— JOB SEARCH PLATFORM —</div>
          <h1 className="text-4xl md:text-5xl font-serif font-bold text-gray-900 mb-6 leading-tight">
            The Essential Platform That Helps You Find Remote Jobs
          </h1>
          <p className="text-lg text-gray-500 mb-10 max-w-2xl mx-auto">
            Discover ultra-fast, dynamic & personalized job opportunities. Search, filter, and apply to jobs from leading companies worldwide.
          </p>
        ...
        <section className="w-full max-w-4xl mx-auto flex flex-wrap justify-center gap-6 mb-12">
          {JOB_CATEGORIES.map((cat, i) => (
            <button
              ...
              className={`flex flex-col items-center justify-center w-44 h-32 bg-white rounded-2xl shadow border border-gray-100 hover:shadow-lg transition-all duration-200 group relative ${selectedCategories.includes(cat.name) ? 'ring-2 ring-blue-400 border-blue-200' : ''}`}
            >
              <div className="text-3xl mb-2">{cat.icon}</div>
              <div className="text-base font-semibold text-gray-800 mb-1">{cat.name}</div>
        ...
        <section className="w-full flex flex-col items-center mb-8">
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-2 items-center justify-center w-full max-w-xl">
            <input
              type="text"
        ...
        <section className="w-full flex flex-col items-center mb-10">
          ...
          <div className="flex gap-8 text-gray-500 text-sm items-center">
            <span className="flex items-center gap-1"><span className="text-green-500">✓</span> Free Signup</span>
            <span className="flex items-center gap-1"><span className="text-green-500">✓</span> No Credit Card Required</span>
            <span className="flex items-center gap-1"><span className="text-green-500">✓</span> Apply Anytime</span>
          </div>
        </section>
        ...
        <section className="w-full max-w-4xl mx-auto flex flex-wrap justify-center items-center gap-10 py-8 border-t border-gray-100 mb-8">
          {TRUSTED_COMPANIES.map((c) => (
            <div key={c.name} className="text-gray-400 text-xl font-bold tracking-wide opacity-80">
              {c.name}
            </div>
          ))}
        </section>
        ...
        <section className="w-full max-w-7xl mx-auto flex gap-8 items-start">
          ...
          <aside className="w-72 bg-white rounded-2xl shadow border border-gray-100 p-6 flex flex-col gap-8 sticky top-8 self-start min-h-[400px]">
            <div>
              ...
              <div className="flex flex-col gap-2 text-gray-700 text-base">
                {JOB_TYPES.map(type => (
                  <label key={type.value}><input type="checkbox" className="mr-2" checked={selectedTypes.includes(type.value)} onChange={() => handleTypeChange(type.value)} /> {type.label}</label>
                ))}
              </div>
              <button className="text-red-500 text-sm mt-2 ml-1" onClick={handleClearAll}>Clear all</button>
            </div>
            <div>
              <label className="flex items-center gap-2 text-gray-700 text-base mt-2">
                <input type="checkbox" checked={hideNoSalary} onChange={e => setHideNoSalary(e.target.checked)} />
                Hide jobs with no salary
              </label>
            </div>
            <div>
            ...
            <div>
              ...
              <input type="range" min={SALARY_MIN} max={SALARY_MAX} value={salaryRange[1]} onChange={handleSalaryChange} className="w-full" />
            ...
            <div>
              ...
              <div className="flex flex-col gap-2 text-gray-700 text-base">
                {JOB_CATEGORIES.map(cat => (
                  <label key={cat.name}><input type="checkbox" className="mr-2" checked={selectedCategories.includes(cat.name)} onChange={() => handleCategoryChange(cat.name)} /> {cat.name}</label>
                ))}
              </div>
            </div>
          </aside>
          ...
          <div className="flex-1">
            {geoLoading && (
              <div className="text-center text-gray-500 py-8">Detecting your country...</div>
            )}
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-gray-900">Recommended jobs</h2>
              <div className="relative">
                <select className="appearance-none border border-gray-200 rounded-full px-4 py-2 pr-8 text-gray-700 bg-white font-medium shadow-sm focus:outline-none" value={sort} onChange={handleSortChange}>
            ...
            {loading && <div className="text-center text-gray-500 py-8">Loading jobs...</div>}
            {error && <div className="text-center text-red-500 py-8">{error}</div>}
            {!loading && !error && filteredJobs.length === 0 && (
              <div className="text-center text-gray-400 py-8 text-lg">No jobs found. Try a different search.</div>
            )}
            <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {paginatedJobs.map(job => {
                ...
                const isEntryLevel = entryKeywords.some(kw =>
                  `${job.title} ${(job.tags || []).join(' ')} ${job.description}`.toLowerCase().includes(kw)
                );
                // Salary display
                const hasSalaryDigits = /\d/.test(job.salary || '');
                const salaryDisplay = hasSalaryDigits ? job.salary : 'Not Given';
                // Location display
                const locationDisplay = job.candidate_required_location || 'Remote';
                // Date display
                const dateDisplay = new Date(job.publication_date).toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' });
                // Badges
                const badges = [];
                ...
                return (
                  <li
                    key={job.id}
                    className={`relative rounded-3xl border border-gray-200 shadow-md p-6 flex flex-col min-h-[340px] ${getPastelBg(job.category)}`}
                  >
                    ...
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <button className="w-8 h-8 flex items-center justify-center rounded-full bg-white/80 border border-gray-100 shadow-sm hover:bg-gray-100 transition">
                        <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 5v14l7-5 7 5V5a2 2 0 00-2-2H7a2 2 0 00-2 2z" /></svg>
                      </button>
                    </div>
                    ...
                    <div className="flex items-center gap-3 mb-2">
                      <div className="w-10 h-10 flex items-center justify-center rounded-full bg-white border border-gray-200 shadow-sm">
                        {job.company_logo_url ? (
                          <img src={job.company_logo_url} alt={job.company_name} className="w-8 h-8 object-contain rounded-full" />
                        ) : (
                          <span className="text-lg font-bold text-gray-700">{job.company_name[0]}</span>
                        )}
                      </div>
                      <div className="flex-1">
                        <div className="text-xs text-gray-500 font-semibold leading-tight">{job.company_name}</div>
                        <div className="text-lg font-bold text-gray-900 leading-tight">{job.title}</div>
                      </div>
                    </div>
                    {/* Badges or Description */}
                    {job.description && !job.description.includes('<') && !job.description.includes('>') ? (
                      <div className="text-gray-600 text-sm line-clamp-2 mb-4 mt-2">{job.description}</div>
                    ) : (
                      <div className="flex flex-wrap gap-2 mb-4 mt-2">
                        {badges.map((badge, i) => (
                          <span key={i} className="inline-block bg-white/80 border border-gray-200 text-gray-700 text-xs px-2 py-0.5 rounded-full font-medium shadow-sm">
                            {badge}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="flex items-end justify-between mt-auto pt-4">
                      <div>
                        <div className="text-gray-900 font-bold text-base">{salaryDisplay}</div>
                        <div className="text-xs text-gray-500 mt-1">{locationDisplay}</div>
                      </div>
                      <a
                        href={job.url}
                        target="_blank"
